...
{
    uint16_t i;
    struct sol_http_response_progressive *sse;
    static const struct sol_str_slice begin =
        SOL_STR_SLICE_STR("data: ", sizeof("data: ") - 1);
    static const struct sol_str_slice end =
        SOL_STR_SLICE_STR("\n\n", sizeof("\n\n") - 1);
    ...
    if (flags & SOL_FD_FLAGS_IN) {
        ...
        value.used = 0;
        err = sol_buffer_set_slice(&value, begin);
        if (err < 0) {
    ...
    SOL_PTR_VECTOR_FOREACH_IDX (&responses, sse, i)
        sol_http_response_progressive_feed(sse, sol_buffer_get_slice(&value));
...
static void
delete_cb(void *data, const struct sol_http_response_progressive *sse)
{
...
request_events_cb(void *data, struct sol_http_request *request)
{
    ...
    ret = sol_http_response_set_progressive_headers(&response);
    if (ret < 0)
        return ret;

    sse = sol_http_server_send_response_progressive(request, &response, delete_cb, NULL);
    ...
    if (ret < 0) {
        sol_http_response_progressive_del(sse, false);
        return ret;
    }
