...
static struct sol_ptr_vector responses;
...
on_stdin(void *data, int fd, uint32_t flags)
{
    ...
    if (flags & SOL_FD_FLAGS_IN) {
        ...
        value.used = 0;
        err = sol_buffer_insert_bytes(&value, 0, (uint8_t *)"data: ", sizeof("data: ") - 1);
        if (err < 0) {
...
startup_server(void)
{
    ...
    c = sol_buffer_set_slice(&value,
        sol_str_slice_from_str("data: Soletta string server, set the value using the keyboard\n\n"));
    if (c < 0) {
