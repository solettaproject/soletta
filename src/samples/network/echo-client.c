...
startup_client(void)
{
    ...
    while ((c = getopt_long(argc, argv, "a:h", opts, &opt_idx)) != -1) {
        switch (c) {
        case 'a':
            addr = optarg;
            break;
    ...
    sock = sol_socket_new((struct sol_socket_options *)&options);
    if (!sock) {
        fprintf(stderr, "ERROR: Could not create the socket\n");
        goto err;
    }

    address.family = SOL_NETWORK_FAMILY_INET6;
    ...
    len = (size_t)(itr - addr);
    memcpy(addr_str, addr, len);

    if (!sol_network_link_addr_from_str(&address, addr_str)) {
        fprintf(stderr, "ERROR: Could not convert the address: %s\n", addr_str);
        goto err;
    }

    itr++;
    address.port = atoi(itr);
