...
/* TODO would be lovely to have this available globally... create a new
* blob from some variable - but duplicating it, not just wrapping it */
...
#define UPDATE_BLOB(_blob, _val) \
    memcpy(_blob->mem, _val, sizeof(*_val));


#define SEND_BLOB(_blob, _id, _node) \
    do { \
        r = sol_ipm_send(_id, _blob, NULL, NULL); \
        if (r < 0) \
...
boolean_writer_open(struct sol_flow_node *node, void *data, const struct sol_flow_node_options *options)
{
    ...
    SOL_INT_CHECK(opts->id, > sol_ipm_max_id(), -EINVAL);
...
boolean_writer_process(struct sol_flow_node *node, void *data, uint16_t port, uint16_t conn_id, const struct sol_flow_packet *packet)
{
    ...
    CREATE_BLOB(&in_value);

    r = sol_flow_packet_get_boolean(packet, &in_value);
    SOL_INT_CHECK(r, < 0, r);

    UPDATE_BLOB(blob, &in_value);
...
string_writer_process(struct sol_flow_node *node, void *data, uint16_t port, uint16_t conn_id, const struct sol_flow_packet *packet)
{
    ...
    blob = sol_blob_new(SOL_BLOB_TYPE_DEFAULT, NULL, str, strlen(str) + 1);
    SOL_NULL_CHECK_GOTO(str, fail);
