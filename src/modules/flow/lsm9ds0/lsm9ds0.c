...
struct lsm9ds0_gyro_data {
    ...
    struct sol_i2c *i2c;
    struct sol_vector init_q;
    struct sol_vector read_q;
    uint32_t scale;
    uint8_t addr;
    bool init;
};
...
_read_gyro_done(void *cb_data, ssize_t status)
{
    ...
    _parse_raw_data(&gyro, sol_vector_get_no_check(&mdata->read_q, 0), (mdata->scale / 32768.0));
...
_init_gyro_queues(struct lsm9ds0_gyro_data *mdata, uint8_t scale)
{
    ...
    sol_vector_init(&mdata->read_q, sizeof(struct sol_i2c_op));
    op = sol_vector_append_n(&mdata->read_q, 6);
    ...
    // Init
    sol_vector_init(&mdata->init_q, sizeof(struct sol_i2c_op));
    op = sol_vector_append_n(&mdata->init_q, 5);
...
lsm9ds0_gyro_open(struct sol_flow_node *node, void *data, const struct sol_flow_node_options *options)
{
    ...
    mdata->pending = sol_i2c_dispatcher_add_op_set(mdata->i2c, mdata->addr, &mdata->init_q,
...
lsm9ds0_gyro_close(struct sol_flow_node *node, void *data)
{
    ...
    sol_vector_clear(&mdata->init_q);
    sol_vector_clear(&mdata->read_q);
}
    ...
    const struct sol_flow_packet *packet)
{
    ...
    mdata->pending = sol_i2c_dispatcher_add_op_set(mdata->i2c, mdata->addr, &mdata->read_q,
...
_read_xmt_done(void *cb_data, ssize_t status)
{
    ...
    _parse_raw_data(&xmt, sol_vector_get_no_check(&mdata->read_q, 0), (mdata->accel_scale / 32768.0));
    sol_flow_send_direction_vector_packet(mdata->node,
        SOL_FLOW_NODE_TYPE_LSM9DS0_XMT__OUT__ACCEL, &xmt);

    _parse_raw_data(&xmt, sol_vector_get_no_check(&mdata->read_q, 6), (mdata->mag_scale / 32768.0));
    sol_flow_send_direction_vector_packet(mdata->node,
        SOL_FLOW_NODE_TYPE_LSM9DS0_XMT__OUT__MAG, &xmt);

    t_op = sol_vector_get_no_check(&mdata->read_q, 13);
...
_init_xmt_queues(struct lsm9ds0_xmt_data *mdata, uint8_t accel_reg, uint8_t mag_reg)
{
    ...
    sol_vector_init(&mdata->read_q, sizeof(struct sol_i2c_op));
    op = sol_vector_append_n(&mdata->read_q, 14);
    // Accel X Y Z
    op[0].type = SOL_I2C_READ;
    op[0].reg = OUT_X_L_A;
    op[1].type = SOL_I2C_READ;
    ...
    // Init
    sol_vector_init(&mdata->init_q, sizeof(struct sol_i2c_op));
    op = sol_vector_append_n(&mdata->init_q, 6);
