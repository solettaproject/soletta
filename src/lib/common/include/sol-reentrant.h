 ...
 *
 * This structure is meant to be used as the first field of larger structures
 * which are affected by calls to external callbacks that in turn end up
 * calling library APIs. The possibility of a double free is particularly
 ...
 * the reentrant structure from the recorded value. Unlike a monitor, which
 * protects the list of callbacks itself, this wrapper protects the library's
 * structure from being deleted as part of the external callback, if it's still
 * in use after the callback completes.
 */
