    ...
    const uint8_t *data, uint16_t datalen)
{
    ...
    sol_ptr_vector_append(&socket->pending_buffers, buf);

    process_post(&soletta_app_process, sol_udp_socket_event, socket);
}
...
sol_socket_ip_default_new(const struct sol_socket_options *options)
{
    ...
    sol_mainloop_contiki_event_handler_add(&sol_udp_socket_event, socket,
        receive_process_cb, NULL);
