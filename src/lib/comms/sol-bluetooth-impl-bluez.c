...
SOL_API int
sol_bt_register_agent(const struct sol_bt_agent *agent, const void *data)
{
    struct context *ctx = &context;
    sd_bus *bus = sol_bus_client_get_bus(ctx->bluez);
    const char *service = sol_bus_client_get_service(ctx->bluez);
...
SOL_API int
sol_bt_agent_finish_passkey_entry(struct sol_bt_conn *conn, uint32_t passkey)
{
    struct context *ctx = &context;
    const struct sol_bt_agent *agent = ctx->agent;
    int r;
...
SOL_API int
sol_bt_agent_finish_cancel(struct sol_bt_conn *conn)
{
    struct context *ctx = &context;
    sd_bus_message *reply;
    int r;
...
SOL_API int
sol_bt_agent_finish_passkey_confirm(struct sol_bt_conn *conn)
{
    struct context *ctx = &context;
    const struct sol_bt_agent *agent = ctx->agent;
    int r;
...
SOL_API int
sol_bt_agent_finish_pairing_confirm(struct sol_bt_conn *conn)
{
    struct context *ctx = &context;
    const struct sol_bt_agent *agent = ctx->agent;
    int r;
...
SOL_API int
sol_bt_agent_finish_pincode_entry(struct sol_bt_conn *conn, const char *pin)
{
    struct context *ctx = &context;
    const struct sol_bt_agent *agent = ctx->agent;
    int r;
