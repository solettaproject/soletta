...
#define SOL_SOCKET_DEFAULT_OPTIONS_API_VERSION (1)  /**< compile time API version to be checked during runtime */
    /**
     * must match #SOL_SOCKET_DEFAULT_OPTIONS_API_VERSION at runtime.
     */
 ...
 *
 * @see sol_socket_del()
 * @see sol_socket_default_options
 */
struct sol_socket *sol_socket_new(const struct sol_socket_default_options *options);
 ...
 * available to read the callback set on @c sol_socket_default_options
 ...
 * @param s The value got with @c sol_socket_new.
 * @param on @c true to start to monitor the socket. When the socket is
 * available to write the callback set on @c sol_socket_default_options
