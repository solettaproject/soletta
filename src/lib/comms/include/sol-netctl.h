...
/**
 * @struct sol_connman_network_params
 *
...
enum sol_connman_service_state {
     ...
     * will become invalid.
     */
    SOL_CONNMAN_SERVICE_STATE_REMOVE,
};
...
/**
 * @enum sol_connman_error
 *
 * @brief the connection manager action error
 *
 */
enum sol_connman_error {
    /*
     * @brief the error is none
     *
     * The error is when the action is no error.
     */
    SOL_CONNMAN_ERROR_NONE = 0,
    /*
     * @brief the error is no mem
     *
     * The error is when the system is no memory.
     */
    SOL_CONNMAN_ERROR_NO_MEM,
    /*
     * @brief the error is access denied
     *
     * The error is when the action is access denied.
     */
    SOL_CONNMAN_ERROR_ACCESS_DENIED,
    /*
     * @brief the error is invalid args
     *
     * The error is when invalid argument.
     */
    SOL_CONNMAN_ERROR_INVALID_ARGS,
    /*
     * @brief the error is process id unknown
     *
     * The error is when no such process.
     */
    SOL_CONNMAN_ERROR_UNIX_PROCESS_ID_UNKNOWN,
    /*
     * @brief the error is file not found
     *
     * The error is when file not found.
     */
    SOL_CONNMAN_ERROR_FILE_NOT_FOUND,
    /*
     * @brief the error is file exists
     *
     * The error is when file exists.
     */
    SOL_CONNMAN_ERROR_FILE_EXISTS,
    /*
     * @brief the error is time out
     *
     * The error is when the action is timed out.
     */
    SOL_CONNMAN_ERROR_TIMEOUT,
    /*
     * @brief the error is io error
     *
     * The error is when input/output error.
     */
    SOL_CONNMAN_ERROR_IO_ERROR,
    /*
     * @brief the error is disconnected
     *
     * The error is when the action is disconnected.
     */
    SOL_CONNMAN_ERROR_DISCONNECTED,
    /*
     * @brief the error is not supported
     *
     * The error is when the action is not supported.
     */
    SOL_CONNMAN_ERROR_NOT_SUPPORTED,
    /*
     * @brief the error is bad address
     *
     * The error is when address not available.
     */
    SOL_CONNMAN_ERROR_BAD_ADDRESS,
    /*
     * @brief the error limits exceeded
     *
     * The error is when limits exceeded.
     */
    SOL_CONNMAN_ERROR_LIMITS_EXCEEDED,
    /*
     * @brief the error address in use
     *
     * The error is when address in use.
     */
    SOL_CONNMAN_ERROR_ADDRESS_IN_USE,
    /*
     * @brief the error inconsistent message
     *
     * The error is when inconsistent message.
     */
    SOL_CONNMAN_ERROR_INCONSISTENT_MESSAGE,
    /*
     * @brief the error unknown
     *
     * The error is when the action is unknown error.
     */
    SOL_CONNMAN_ERROR_UNKNOWN,
};
 ...
 */
typedef void (*sol_connman_service_monitor_cb)
    (void *data,
 ...
 * @param data the user data
 */
typedef void (*sol_connman_manager_monitor_cb)(void *data);
 ...
 */
typedef void (*sol_connman_error_monitor_cb)
    (void *data, const struct sol_connman_service *service,
    enum sol_connman_error error);
 ...
 */
int sol_connman_service_get_network_address(
    const struct sol_connman_service *service, struct sol_network_link **link);
 ...
 * This callback of sol_connman_add_manager_monitor must be
 * added BEFORE sol_connman_get_state() is called
 ...
 * @return 0 on success, -errno on failure.
 */
int sol_connman_set_radios_offline(bool enabled);
 ...
 * after the state is to give by manager monitor.
 * This callback of sol_connman_add_manager_monitor must be
 * added BEFORE sol_connman_get_radios_offline() is called
 * to ensure no messages are lost.
 *
 * @return True if the offline is enabled, false otherwise.
 */
bool sol_connman_get_radios_offline(void);
 ...
 * via sol_connman_add_service_monitor() callbacks. The service connect error info will be
 * notified by error monitor. This callbacks of sol_connman_add_service_monitor and
 * sol_connman_add_error_monitor must be added BEFORE the sol_connman_service_connect()
 ...
 * but just some dispatching/immediate error. The actual state change will be notified
 * via sol_connman_add_service_monitor() callbacks.The service disconnect error info
 * will be notified by error monitor. This callbacks of sol_connman_add_service_monitor and
 * sol_connman_add_error_monitor must be added BEFORE the sol_connman_service_disconnect()
 * is called to ensure no messages are lost.
 *
 ...
 * sol_connman_add_service_monitor will trigger all required activations.
 ...
 */
int sol_connman_add_service_monitor(sol_connman_service_monitor_cb cb,
    const void *data);
 ...
 * sol_connman_del_service_monitor will trigger all required activations.
 ...
 * @return 0 on success, -errno on failure.
 */
int sol_connman_del_service_monitor(sol_connman_service_monitor_cb cb,
    const void *data);

/**
 * @brief Adds a monitor for the updated connman manager properties
 *
 * To monitor (State, Offline...), a single monitor can be done for
 * "something changed", and then the new value can be get.
 * sol_connman_add_manager_monitor will trigger all required activations.
 * This callback must be added BEFORE sol_connman_get_state and
 * sol_connman_get_radios_offline.
 *
 ...
 * @return 1 on the first time success, 0 on success, -errno on failure.
 */
int sol_connman_add_manager_monitor(sol_connman_manager_monitor_cb cb,
    const void *data);

/**
 * @brief Dels a monitor for the updated connman manager properties
 *
 * Removes the monitor for updated connman manager properties.
 * sol_connman_del_manager_monitor will trigger all required activations.
 *
 * @param cb monitor Callback to be called when the manager are updated
 * @param data Add a user data per callback
 *
 * @return 0 on success, -errno on failure.
 */
int sol_connman_del_manager_monitor(sol_connman_manager_monitor_cb cb,
    const void *data);
 ...
 * sol_connman_add_error_monitor will call trigger all required activations.
 * This callback must be added BEFORE sol_connman_service_connect and
 * sol_connman_service_disconnect.
 *
 ...
 * @return 0 on success, -errno on failure.
 */
int sol_connman_add_error_monitor(sol_connman_error_monitor_cb cb,
    const void *data);
 ...
 * sol_connman_del_error_monitor will trigger all required activations.
 *
 * @param cb monitor Callback to be called when the call result are updated
 * @param data Add a user data per callback
 *
 * @return 0 on success, -errno on failure.
 */
int sol_connman_del_error_monitor(sol_connman_error_monitor_cb cb,
    const void *data);
/**
 * @brief Gets the connman services
 *
 * The connman services vector can be given via sol_connman_get_services.
 ...
 */
const struct sol_ptr_vector *sol_connman_get_services(void);
