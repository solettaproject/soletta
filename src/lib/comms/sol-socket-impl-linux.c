...
on_socket_event(void *data, int fd, unsigned int flags)
{
    ...
    } else if (flags & SOL_FD_FLAGS_OUT) {
...
struct sol_socket *
sol_socket_default_new(const struct sol_socket_options *options)
{
    int ret;
    ...
    SOL_SOCKET_OPTIONS_CHECK_SUB_API_VERSION(options, SOL_SOCKET_IP_OPTIONS_API_VERSION, NULL);
