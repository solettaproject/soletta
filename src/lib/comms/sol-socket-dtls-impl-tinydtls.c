...
call_user_read_cb(struct dtls_context_t *ctx, session_t *session, uint8_t *buf, size_t len)
{
    ...
    if (socket->read.cb(socket->data, socket))
        return len;
...
struct sol_socket *
sol_socket_default_dtls_new(const struct sol_socket_default_options *options)
{
    ...
    struct sol_socket_default_options opts = *options;
