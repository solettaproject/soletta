...
request_new(sol_coap_method_t method, sol_coap_msgtype_t type, struct sol_oic_resource *res, bool isObserve)
{
    ...
    if (!_set_token_and_mid(request->base.pkt, &request->token))
        goto error;
    ...
    if (sol_coap_packet_add_uri_path_option(request->base.pkt, href) < 0) {
