    ...
    char *psk, size_t psk_len)
{
    ...
    r = sol_buffer_append_printf(&buf, "%s", OIC_CRED_FILE_PREFIX);
    SOL_INT_CHECK(r, < 0, -ENOBUFS);
    r = sol_buffer_append_as_base64(&buf, id, SOL_BASE64_MAP);
    SOL_INT_CHECK(r, < 0, -ENOBUFS);
    r = sol_buffer_append_printf(&buf, "%s", OIC_CRED_FILE_SUFIX);
    SOL_INT_CHECK(r, < 0, -ENOBUFS);

    cert = sol_cert_load_from_file(buf.data);
    if (!cert)
        return -ENOENT;

    sol_buffer_reset(&buf);

    contents = sol_cert_get_contents(cert);
    sol_cert_unref(cert);

    if (contents->size < PSK_BASE64_LEN) {
        sol_blob_unref(contents);
        return -ENOENT;
    }

    r = sol_buffer_append_from_base64(&buf,
        SOL_STR_SLICE_STR(contents->mem, PSK_BASE64_LEN), SOL_BASE64_MAP);
    sol_blob_unref(contents);
    SOL_INT_CHECK(r, < 0, -ENOBUFS);
