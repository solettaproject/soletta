...
sol_bt_uuid_to_str(const struct sol_bt_uuid *uuid, struct sol_buffer *buffer)
{
    ...
    memcpy(&tmp0, &uuid->val128[0], sizeof(tmp0));
    memcpy(&tmp1, &uuid->val128[4], sizeof(tmp1));
    memcpy(&tmp2, &uuid->val128[6], sizeof(tmp2));
    memcpy(&tmp3, &uuid->val128[8], sizeof(tmp3));
    memcpy(&tmp4, &uuid->val128[10], sizeof(tmp4));
    memcpy(&tmp5, &uuid->val128[14], sizeof(tmp5));
