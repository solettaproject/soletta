...
sol_flow_parse_string(
    ...
    const char *filename)
{
    struct sol_buffer buf = SOL_BUFFER_INIT_CONST((char *)cstr, strlen(cstr));
    ...
    const char *filename)
{
    ...
    buf = SOL_BUFFER_INIT_CONST((char *)str, strlen(str));

    return sol_flow_parse_buffer_metatype(parser, metatype, &buf, filename);
}
