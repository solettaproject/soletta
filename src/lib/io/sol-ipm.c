...
static struct sol_vector ipm_consumed_handlers = SOL_VECTOR_INIT(struct sol_ipm_consumed_handler);
...
extern unsigned int heap_sz, heap_max;
...
ipm_reaper_process(void *data)
{
    ...
    SOL_VECTOR_FOREACH_IDX (&ipm_consumed_handlers, handler, idx) {
        if (handler->id == message->id) {
            ...
            free(message); /* This blob is SOL_BLOB_TYPE_NO_FREE, so we need to
                              free it after using it. Parent takes care of freeing
                              blob->mem*/
            return;
        }
    }
    ...
    const void *data)
{
    ...
    SOL_NULL_CHECK(receive_cb, -EEXIST);
    ...
    const void *data)
{
    ...
    SOL_VECTOR_FOREACH_IDX (&ipm_consumed_handlers, handler, idx) {
        if (handler->id == id && message_consumed_cb)
            return -EEXIST;
    ...
    SOL_NULL_CHECK(message_consumed_cb, -EEXIST);
