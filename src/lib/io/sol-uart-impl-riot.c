...
sol_uart_open(const char *port_name, const struct sol_uart_config *config)
{
    ...
    sol_buffer_init_flags(&uart->rx_buffer, NULL, 0,
        SOL_BUFFER_FLAGS_DEFAULT | SOL_BUFFER_FLAGS_NO_NUL_BYTE);
...
sol_uart_read(struct sol_uart *uart, struct sol_buffer *buf)
{
    ...
    if (buf->flags & SOL_BUFFER_FLAGS_FIXED_CAPACITY) {
        ...
        if (!min)
            return -ENOMEM;
        r = sol_buffer_append_bytes(buf, uart->rx_buffer.data, min);
        SOL_INT_CHECK(r, < 0, r);
    ...
    memcpy(buf, &uart->rx_buffer, sizeof(struct sol_buffer));
