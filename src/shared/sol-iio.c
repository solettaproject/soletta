#include <ctype.h>
#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
#include <limits.h>
#include <stdio.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>

#include <sol-log.h>
#include <sol-mainloop.h>
#include <sol-str-slice.h>
#include <sol-util.h>
#include <sol-util-linux.h>
#include <sol-vector.h>

#include "sol-iio.h"

struct sol_iio_device {
    char *trigger_name;
    sol_iio_reader_cb reader_cb;
    void *reader_cb_data;
    struct sol_fd *fd_handler;
    struct sol_ptr_vector channels;
    int device_id;
    int trigger_id;
    int fd;
    bool buffer_enabled : 1;
    bool manual_triggering : 1;
};

struct sol_iio_channel {
    struct sol_iio_device *device;
    char *name;
    double scale;
    int index;
    int offset;

    int storagebits;
    int bits;
    int shift;
    int offset_in_buffer;

    bool little_endian : 1;
    bool is_signed : 1;
};

struct sol_iio_buffer {
    size_t size;
    uint8_t *data;
};

#define DEVICE_PATH "/dev/iio:device%d"
#define SYSFS_DEVICES_PATH "/sys/bus/iio/devices"
#define BUFFER_ENABLE_DEVICE_PATH "/sys/bus/iio/devices/iio:device%d/buffer/enable"
#define BUFFER_LENGHT_DEVICE_PATH "/sys/bus/iio/devices/iio:device%d/buffer/length"
#define CURRENT_TRIGGER_DEVICE_PATH "/sys/bus/iio/devices/iio:device%d/trigger/current_trigger"

#define SYSFS_TRIGGER_NOW_PATH "/sys/bus/iio/devices/%s/trigger_now"
#define SYSFS_TRIGGER_NOW_BY_ID_PATH "/sys/bus/iio/devices/trigger%d/trigger_now"
#define SYSFS_TRIGGER_NAME_PATH "/sys/bus/iio/devices/%s/name"
#define SYSFS_TRIGGER_SYSFS_ADD_TRIGGER "/sys/bus/iio/devices/iio_sysfs_trigger/add_trigger"
#define SYSFS_TRIGGER_SYSFS_PATH "/sys/bus/iio/devices/iio_sysfs_trigger"

#define CHANNEL_RAW_PATH "/sys/bus/iio/devices/iio:device%d/%s_raw"
#define CHANNEL_OFFSET_PATH "/sys/bus/iio/devices/iio:device%d/%s_offset"
#define CHANNEL_SCALE_PATH "/sys/bus/iio/devices/iio:device%d/%s_scale"

#define CHANNEL_SCAN_ENABLE_PATH "/sys/bus/iio/devices/iio:device%d/scan_elements/%s_en"
#define CHANNEL_SCAN_INDEX_PATH "/sys/bus/iio/devices/iio:device%d/scan_elements/%s_index"
#define CHANNEL_SCAN_TYPE_PATH "/sys/bus/iio/devices/iio:device%d/scan_elements/%s_type"

#define SAMPLING_FREQUENCY_DEVICE_PATH "/sys/bus/iio/devices/iio:device%d/sampling_frequency"
#define SAMPLING_FREQUENCY_BUFFER_PATH "/sys/bus/iio/devices/iio:device%d/buffer/sampling_frequency"
#define SAMPLING_FREQUENCY_TRIGGER_PATH "/sys/bus/iio/devices/trigger%d/sampling_frequency"

static bool
craft_filename_path(char *path, size_t size, const char *base, ...)
{
    va_list args;
    int len;

    va_start(args, base);
    len = vsnprintf(path, size, base, args);
    va_end(args);

    return len >= 0 && len <= PATH_MAX;
}

static bool
write_file(const char *path, const char *format, ...)
{
    va_list args;
    char *content;
    int fd;
    size_t len;
    bool result = true;

    va_start(args, format);
    vasprintf(&content, format, args);
    va_end(args);

    fd = open(path, O_WRONLY | O_CLOEXEC);
    if (fd < 0)
        result = false;

    if (result) {
        len = strlen(content);
        result = write(fd, content, len) == (ssize_t)len;
    }

    free(content);
    close(fd);

    return result;
}

static bool
check_trigger_now_file(const char *trigger_dir)
{
    int len;
    char path[PATH_MAX + 1];
    struct stat st;

    len = snprintf(path, sizeof(path), SYSFS_TRIGGER_NOW_PATH, trigger_dir);

    // TODO check permissions of file trigger_now too, not only existence
    return len > 0 && len <= PATH_MAX && !stat(path, &st);
}

static bool
check_trigger_name(const char *trigger_dir, const char *trigger_name)
{
    char path[PATH_MAX + 1];
    char *name;
    int len;
    bool result;

    len = snprintf(path, sizeof(path), SYSFS_TRIGGER_NAME_PATH, trigger_dir);
    if (len < 0 || len > PATH_MAX) {
        SOL_WRN("Could not open IIO trigger (%s) name on sysfs", trigger_dir);
        return false;
    }

    len = sol_util_read_file(path, "%ms", &name);
    if (len < 0) {
        SOL_WRN("Could not read IIO trigger (%s) name on sysfs", trigger_dir);
        return false;
    }

    result = streq(trigger_name, name);
    free(name);

    return result;
}

static bool
check_manual_triggering(struct sol_iio_device *device)
{
    DIR *dir;
    const struct dirent *ent;

    /* The only way I've found to relact trigger name and trigger is
     * by opening all triggers on /sys/bus/iio/devices
     * and checking name by name =/ */

    dir = opendir(SYSFS_DEVICES_PATH);
    if (!dir) {
        SOL_WRN("No IIO devices available");
        return false;
    }

    while (ent = readdir(dir), ent != NULL) {
        if (strstr(ent->d_name, "trigger") == ent->d_name) {
            if (check_trigger_name(ent->d_name, device->trigger_name)) {
                device->manual_triggering = check_trigger_now_file(ent->d_name);
                /* triggers dirs are of the form triggerX, so here we save X */
                device->trigger_id = atoi(ent->d_name + strlen("trigger"));
                return true;
            }
        }
    }

    return false;
}

static bool
set_current_trigger(struct sol_iio_device *device, const char *trigger_name)
{
    char path[PATH_MAX + 1];
    int len;

    len = snprintf(path, sizeof(path), CURRENT_TRIGGER_DEVICE_PATH, device->device_id);
    if (len < 0 || len > PATH_MAX) {
        SOL_WRN("Could not open device current_trigger file");
        return false;
    }

    if (!write_file(path, "%s", trigger_name)) {
        SOL_WRN("Could not write to device current_trigger file");
        return false;
    }

    return true;
}

static bool
create_sysfs_trigger(struct sol_iio_device *device)
{
    int len, i;
    struct stat st;
    char *trigger_name;
    bool result = false;

    /* First, check if 'iio_sysfs_trigger' is available */
    if (stat(SYSFS_TRIGGER_SYSFS_ADD_TRIGGER, &st)) {
        SOL_WRN("No 'iio_sysfs_tigger' under '/sys/bus/iio/devices'."
            " Missing 'modprobe iio-trig-sysfs'?");
        return false;
    }

    /* TODO check available trigger indice instead of using a random one */
    /* Create new trigger */
    i = rand();
    if (!write_file(SYSFS_TRIGGER_SYSFS_ADD_TRIGGER, "%d", i))
        goto error;

    /* Set device current trigger */
    len = asprintf(&trigger_name, "sysfstrig%d", i);
    if (len < 0)
        goto error;
    result = set_current_trigger(device, trigger_name);

    if (result)
        device->trigger_name = strdup(trigger_name);

    free(trigger_name);

    return result;

error:
    SOL_WRN("Could not create sysfs trigger.");

    return false;
}

static bool
check_trigger(struct sol_iio_device *device)
{
    char path[PATH_MAX + 1];
    char *name = NULL;
    int len;

    len = snprintf(path, sizeof(path), CURRENT_TRIGGER_DEVICE_PATH, device->device_id);
    if (len < 0 || len > PATH_MAX) {
        SOL_WRN("Could not open IIO device%d trigger on sysfs", device->device_id);
        return false;
    }

    len = sol_util_read_file(path, "%ms", &name);
    if (len < 0) {
        SOL_INF("No current trigger for iio:device%d. Creating a sysfs one.",
            device->device_id);
        if (!create_sysfs_trigger(device))
            return false;
    } else
        device->trigger_name = name;

    return check_manual_triggering(device);
}

static void
set_buffer_size(struct sol_iio_device *device, int buffer_size)
{
    char path[PATH_MAX + 1];

    if (craft_filename_path(path, sizeof(path), BUFFER_LENGHT_DEVICE_PATH, device->device_id)) {
        SOL_WRN("Could not set IIO device buffer size");
        return;
    }

    if (!write_file(path, "%d", buffer_size)) {
        SOL_WRN("Could not set IIO device buffer size");
    }

    return;
}

static bool
set_buffer_enabled(struct sol_iio_device *device, bool enabled)
{
    char path[PATH_MAX + 1];
    int len;

    len = snprintf(path, sizeof(path), BUFFER_ENABLE_DEVICE_PATH, device->device_id);
    if (len < 0 || len > PATH_MAX)
        return false;

    if (!write_file(path, "%d", enabled))
        return false;

    return true;
}

static size_t
calc_buffer_size(struct sol_iio_device *device)
{
    struct sol_iio_channel *channel;
    int i;
    size_t size = 0;

    SOL_PTR_VECTOR_FOREACH_IDX (&device->channels, channel, i) {
        size += channel->storagebits;
    }

    return size / 8;
}

static bool
device_reader_cb(void *data, int fd, unsigned int active_flags)
{
    struct sol_iio_device *device = data;
    bool result = true;
    size_t len = calc_buffer_size(device);
    uint8_t *buffer = NULL;

    buffer = calloc(1, len);
    if (!buffer) {
        SOL_WRN("Could not allocate memory for read buffer");
        result = false;
    }

    if (result && active_flags & (SOL_FD_FLAGS_ERR | SOL_FD_FLAGS_HUP | SOL_FD_FLAGS_NVAL)) {
        SOL_WRN("Unexpected reading");
        result = false;
    }

    while (result) {
        ssize_t ret;

        ret = read(fd, buffer, len);
        if (ret < 0) {
            if (errno == EINTR)
                continue;
            else if (errno == EAGAIN || errno == EWOULDBLOCK)
                break;
            else {
                result = false;
                break;
            }
        }

        if ((size_t)ret == len) {
            if (device->reader_cb) {
                struct sol_iio_buffer iio_buffer;

                iio_buffer.size = len;
                iio_buffer.data = buffer;
                device->reader_cb(device, &iio_buffer, device->reader_cb_data);
            }
        }
    }

    free(buffer);

    return result;
}

static bool
setup_device_reader(struct sol_iio_device *device)
{
    int len, fd;
    char path[PATH_MAX + 1];

    len = snprintf(path, sizeof(path), DEVICE_PATH, device->device_id);
    if (len < 0 || len > PATH_MAX) {
        SOL_WRN("Could not open IIO device%d: Device does not exist", device->device_id);
        return false;
    }

    fd = open(path, O_RDONLY | O_CLOEXEC | O_NONBLOCK);
    if (fd < 0) {
        SOL_WRN("Could not open IIO device%d: Device does not exist", device->device_id);
        return false;
    }

    device->fd_handler = sol_fd_add(fd, SOL_FD_FLAGS_IN | SOL_FD_FLAGS_PRI | SOL_FD_FLAGS_ERR, device_reader_cb, device);
    if (!device->fd_handler) {
        SOL_WRN("Could not setup reader for device%d", device->device_id);
        close(fd);
        return false;
    }

    device->fd = fd;

    return true;
}

struct sol_iio_device *
sol_iio_open(int device_id, int buffer_size, const char *trigger_name)
{
    int len;
    char path[PATH_MAX + 1];
    struct stat st;
    struct sol_iio_device *device = NULL;

    len = snprintf(path, sizeof(path), DEVICE_PATH, device_id);
    if (len < 0 || len > PATH_MAX || stat(path, &st)) {
        SOL_WRN("Could not open IIO device%d: Device does not exist", device_id);
        return NULL;
    }

    device = calloc(1, sizeof(struct sol_iio_device));
    SOL_NULL_CHECK(device, NULL);

    device->device_id = device_id;
    sol_ptr_vector_init(&device->channels);
    device->fd = -1;

    if (buffer_size > -1) {
        if (trigger_name && trigger_name[0] != '\0') {
            if (!set_current_trigger(device, trigger_name)) {
                SOL_WRN("Could not set device current trigger");
                goto error;
            }
        }

        if (!check_trigger(device)) {
            SOL_WRN("No trigger available for device");
            goto error;
        }

        if (buffer_size != 0)
            set_buffer_size(device, buffer_size);

        if (!device->manual_triggering) {
            SOL_WRN("No 'trigger_now' file on device%d current trigger. "
                "It won't be possible to manually trigger a reading on device",
                device->device_id);
        }

        if (!setup_device_reader(device)) {
            SOL_WRN("Could not setup device%d reading", device->device_id);
            goto error;
        }
        device->buffer_enabled = true;
    } else {
        /* buffer_size == -1 means that user doesn't want to use the buffer */
        device->buffer_enabled = false;
        if (!set_buffer_enabled(device, false)) {
            SOL_WRN("Could not enable buffer for device");
            goto error;
        }
    }

    SOL_DBG("iio device created. device%d - buffer_enabled: %d - manual_trigger: %d"
        " - trigger_name: %s - trigger_id: %d", device->device_id,
        device->buffer_enabled, device->manual_triggering, device->trigger_name,
        device->trigger_id);

    return device;

error:
    sol_iio_close(device);

    return NULL;
}

static bool
enable_channel_scan(struct sol_iio_channel *channel)
{
    struct sol_iio_device *device = channel->device;
    char path[PATH_MAX + 1];
    int current_value;

    if (!craft_filename_path(path, sizeof(path), CHANNEL_SCAN_ENABLE_PATH,
        device->device_id, channel->name)) {

        return false;
    }

    /* First, check if not already enabled */
    sol_util_read_file(path, "%d", &current_value);
    if (current_value != 1)
        return write_file(path, "%d", 1);

    return true;
}

static bool
read_channel_scan_info(struct sol_iio_channel *channel)
{
    struct sol_iio_device *device = channel->device;
    char path[PATH_MAX + 1];
    char *type, *original_type;
    int len;
    bool crafted;

    crafted = craft_filename_path(path, sizeof(path), CHANNEL_SCAN_TYPE_PATH,
        device->device_id, channel->name);
    if (!crafted || sol_util_read_file(path, "%ms", &type) < 0)
        return false;

    original_type = type;
    /* Form of type is [be|le]:[s|u]bits/storagebits[>>shift] */
    if (strstr(type, "be:")) {
        channel->little_endian = false;
        type += 3;
    } else if (strstr(type, "le:")) {
        channel->little_endian = true;
        type += 3;
    } else
        channel->little_endian = true;

    if (type[0] == 's') {
        channel->is_signed = true;
        type++;
    } else if (type[0] == 'u') {
        channel->is_signed = false;
        type++;
    } else
        channel->is_signed = true; /* TODO check if signed is the default */

    channel->shift = 0;
    len = sscanf(type, "%d/%d>>%d", &channel->bits, &channel->storagebits, &channel->shift);

    free(original_type);

    if (len < 0)
        return false;

    return true;
}

/* Some channels are named on the form  <type>[_x|_y|_z]
 * This function shall return the name without <x|y|z> components.
 * The form <type>[Y][_modifier] is also common (Y is a number), and this function
 * remove numbers too, in an attempt to get the 'pure' name.
 * TODO there are other esoteric combinations - for them, if we care about,
 * we will probably need API calls to get the 'scale' and 'offset' file names */
static char *
channel_get_pure_name(struct sol_iio_channel *channel)
{
    size_t channel_name_len, i;
    char *channel_pure_name, *channel_name_suffix, *original_channel_pure_name;
    bool modified = false;

    channel_name_len = strlen(channel->name);
    if (channel_name_len > 2) {
        channel_name_suffix = channel->name + channel_name_len - 2;
        if (streq(channel_name_suffix,  "_x") ||
            streq(channel_name_suffix,  "_y") || streq(channel_name_suffix,  "_z")) {

            channel_pure_name = malloc(channel_name_len - 1);
            strncpy(channel_pure_name, channel->name, channel_name_len - 2);
            channel_pure_name[channel_name_len - 2] = '\0';
            return channel_pure_name;
        } else {
            /* Recreate channel name without Y_ components (Y is a number).
             * Idea is 's/[0-9]+//' */
            channel_pure_name = calloc(1, channel_name_len + 1);
            original_channel_pure_name = channel_pure_name;
            for (i = 0; i < channel_name_len; i++) {
                if (isalpha(channel->name[i]) || channel->name[i] == '-' || channel->name[i] == '_')
                    *channel_pure_name++ = channel->name[i];
                else if (isdigit(channel->name[i])) {
                    modified = true;
                    continue;
                }
            }
            if (modified)
                return original_channel_pure_name;
            else
                free(original_channel_pure_name);
        }
    }

    return NULL;
}

static void
channel_get_scale(struct sol_iio_channel *channel)
{
    char path[PATH_MAX + 1];
    int len;
    char *channel_pure_name = NULL;
    struct sol_iio_device *device = channel->device;

#define GET_SCALE(_name) \
    do { \
        len = snprintf(path, sizeof(path), CHANNEL_SCALE_PATH, device->device_id, \
            _name); \
        if (len > 0 && sol_util_read_file(path, "%lf", &channel->scale) > 0) { \
            goto end; \
        } \
    } while (0)

    GET_SCALE(channel->name);
    /* No scale. If channel has x, y, z or Y component, look for scale file without it */
    channel_pure_name = channel_get_pure_name(channel);
    GET_SCALE(channel_pure_name);

    SOL_INF("Could not get scale for channel [%s] in device%d. Assuming 1.0",
        channel->name, device->device_id);
    channel->scale = 1.0;

#undef GET_SCALE

end:
    free(channel_pure_name);
}

static void
channel_get_offset(struct sol_iio_channel *channel)
{
    char path[PATH_MAX + 1];
    int len;
    char *channel_pure_name = NULL;
    struct sol_iio_device *device = channel->device;

#define GET_OFFSET(_name) \
    do { \
        len = snprintf(path, sizeof(path), CHANNEL_OFFSET_PATH, device->device_id, \
            _name); \
        if (len > 0 && sol_util_read_file(path, "%d", &channel->offset) > 0) { \
            goto end; \
        } \
    } while (0)

    GET_OFFSET(channel->name);
    /* No scale. If channel has x, y or z component, look for scale file without it */
    channel_pure_name = channel_get_pure_name(channel);
    GET_OFFSET(channel_pure_name);

    SOL_INF("Could not get offset for channel [%s] in device%d. Assuming 0",
        channel->name, device->device_id);
    channel->offset = 0;

#undef GET_OFFSET

end:
    free(channel_pure_name);
}

static void
iio_del_channel(struct sol_iio_channel *channel)
{
    free(channel->name);

    free(channel);
}

void
sol_iio_close(struct sol_iio_device *device)
{
    int i;
    struct sol_iio_channel *channel;

    SOL_PTR_VECTOR_FOREACH_REVERSE_IDX (&device->channels, channel, i) {
        iio_del_channel(channel);
        sol_ptr_vector_del(&device->channels, i);
    }

    if (device->fd_handler) sol_fd_del(device->fd_handler);
    if (device->fd > -1) close(device->fd);
    free(device->trigger_name);
    free(device);
}

static bool
read_channel_index_in_buffer(struct sol_iio_channel *channel)
{
    struct sol_iio_device *device = channel->device;
    char path[PATH_MAX + 1];
    int len;

    len = snprintf(path, sizeof(path), CHANNEL_SCAN_INDEX_PATH,
        device->device_id, channel->name);
    if (len < 0 || sol_util_read_file(path, "%d", &channel->index) < 0)
        return false;

    return true;
}

struct sol_iio_channel *
sol_iio_add_channel(struct sol_iio_device *device, const char *name)
{
    char path[PATH_MAX + 1];
    struct sol_iio_channel *channel;
    struct stat st;
    int len;

    len = snprintf(path, sizeof(path), CHANNEL_RAW_PATH, device->device_id,
        name);
    if (len < 0 || len > PATH_MAX || stat(path, &st)) {
        SOL_WRN("Could not find channel [%s] for device%d", name,
            device->device_id);
        return NULL;
    }

    channel = calloc(1, sizeof(struct sol_iio_channel));
    SOL_NULL_CHECK(channel, NULL);

    channel->name = strdup(name);
    channel->device = device;

    channel_get_scale(channel);
    channel_get_offset(channel);

    if (device->buffer_enabled) {
        if (!enable_channel_scan(channel)) {
            SOL_WRN("Could not enable scanning of channel [%s] in device%d",
                channel->name, device->device_id);
            goto error;
        }

        if (!read_channel_scan_info(channel)) {
            SOL_WRN("Could not read scanning info of channel [%s] in device%d",
                channel->name, device->device_id);
            goto error;
        }

        if (!read_channel_index_in_buffer(channel)) {
            SOL_WRN("Could not read index in buffer of channel [%s] in device%d",
                channel->name, device->device_id);
            goto error;
        }

        channel->offset_in_buffer = -1;
    }

    sol_ptr_vector_append(&channel->device->channels, channel);

    SOL_DBG("channel %s added. scale: %lf - offset: %d - storagebits: %d",
        channel->name, channel->scale, channel->offset, channel->storagebits);

    return channel;

error:
    iio_del_channel(channel);

    return NULL;
}

bool
sol_iio_read_channel_value(struct sol_iio_channel *channel, double *value)
{
    int len, raw_value;
    char path[PATH_MAX + 1];
    struct sol_iio_device *device = channel->device;

    if (device->buffer_enabled) {
        SOL_WRN("device%d is using buffer - use iio_read_buffer_channel_value() instead",
            device->device_id);
        return false;
    }

    len = snprintf(path, sizeof(path), CHANNEL_RAW_PATH, device->device_id,
        channel->name);
    if (len < 0) {
        SOL_WRN("Could not read channel [%s] in device%d", channel->name,
            device->device_id);
        return false;
    }

    len = sol_util_read_file(path, "%d", &raw_value);
    if (len < 0) {
        SOL_WRN("Could not read channel [%s] in device%d", channel->name,
            device->device_id);
        return false;
    }

    *value = (raw_value + channel->offset) * channel->scale;
    return true;
}

void
sol_iio_set_reader_cb(struct sol_iio_device *device, sol_iio_reader_cb reader_cb, void *data)
{
    device->reader_cb = reader_cb;
    device->reader_cb_data = data;
}

static int
calc_channel_offset_in_buffer(struct sol_iio_channel *channel)
{
    struct sol_iio_channel *channel_itr;
    int i, offset = 0;

    SOL_PTR_VECTOR_FOREACH_IDX (&channel->device->channels, channel_itr, i) {
        if (channel_itr->index < channel->index)
            offset += channel->storagebits;
    }

    return offset;
}

bool
sol_iio_read_buffer_channel_value(struct sol_iio_channel *channel, struct sol_iio_buffer *buffer, double *value)
{
    uint64_t data = 0;
    int64_t s_data = -1;
    int i, j, offset_bytes, storage_bytes;
    bool negative = false;

    /* Leve this calc to the first read, as now we know how much channels are enabled */
    if (channel->offset_in_buffer == -1)
        channel->offset_in_buffer = calc_channel_offset_in_buffer(channel);

    if (channel->offset_in_buffer + channel->storagebits > (int)buffer->size * 8) {
        SOL_WRN("Invalid read on buffer - is buffer complete?");
        return false;
    }

    if (channel->storagebits > 64) {
        // XXX is such a thing even possible?
        SOL_WRN("Could not read channel - more than 64 bits");
        return false;
    }

    offset_bytes = channel->offset_in_buffer / 8;
    storage_bytes = channel->storagebits / 8;
    if (channel->little_endian) {
        for (i = 0, j = 0; i < storage_bytes; i++, j += 8) {
            data |= buffer->data[i + offset_bytes] << j;
        }
    } else {
        for (i = storage_bytes - 1, j = 0; i >= 0; i--, j += 8) {
            data |= buffer->data[i + offset_bytes] << j;
        }
    }

    data >>= channel->shift;

    /* Remove the top useless bits */
    data <<= channel->storagebits - channel->bits;
    data >>= channel->storagebits - channel->bits;

    /* If signed and msb is 1, we have a negative number */
    if (channel->is_signed) {
        negative = data & (1 << (channel->bits - 1));
    }

    if (negative) {
        s_data = data | (s_data << channel->bits);
        *value = (s_data + channel->offset) * channel->scale;
    } else
        *value = (data + channel->offset) * channel->scale;

    return true;
}

bool
sol_iio_device_trigger_now(struct sol_iio_device *device)
{
    char path[PATH_MAX + 1];
    int len;

    if (!device->manual_triggering) {
        SOL_WRN("No manual triggering available for device%d", device->device_id);
        return false;
    }

    len = snprintf(path, sizeof(path), SYSFS_TRIGGER_NOW_BY_ID_PATH,
        device->trigger_id);
    if (len < 0 || len > PATH_MAX) {
        SOL_WRN("No valid trigger_now file available for trigger [%s]",
            device->trigger_name);
        return false;
    }

    if (!write_file(path, "%d", 1)) {
        SOL_WRN("Could not write to trigger_now file for trigger [%s]",
            device->trigger_name);
        return false;
    }

    return true;
}

bool
sol_iio_set_sampling_frequency(struct sol_iio_device *device, int frequency)
{
    char path[PATH_MAX + 1];

    if (craft_filename_path(path, sizeof(path), SAMPLING_FREQUENCY_DEVICE_PATH,
        device->device_id)) {

        if (write_file(path, "%d", frequency))
            return true;
    }

    if (craft_filename_path(path, sizeof(path), SAMPLING_FREQUENCY_BUFFER_PATH,
        device->device_id)) {

        if (write_file(path, "%d", frequency))
            return true;
    }


    if (craft_filename_path(path, sizeof(path), SAMPLING_FREQUENCY_TRIGGER_PATH,
        device->trigger_id)) {

        if (write_file(path, "%d", frequency))
            return true;
    }

    return false;
}

bool
sol_iio_set_channel_scale(struct sol_iio_channel *channel, double scale)
{
    char path[PATH_MAX + 1], *pure_name = NULL;
    bool result;

    if (craft_filename_path(path, sizeof(path), CHANNEL_SCALE_PATH,
        channel->device->device_id, channel->name)) {

        if (write_file(path, "%lf", scale)) {
            channel->scale = scale;
            return true;
        }
    }

    /* If failed, try channel pure name */
    pure_name = channel_get_pure_name(channel);
    if (pure_name && craft_filename_path(path, sizeof(path), CHANNEL_SCALE_PATH,
        channel->device->device_id, channel->name)) {

        result = (write_file(path, "%lf", scale));
        if (result)
            channel->scale = scale;
    }

    if (!result)
        SOL_WRN("Could not set scale to %lf on channel [%s] of device%d",
            scale, channel->name, channel->device->device_id);

    free(pure_name);

    return result;
}

bool
sol_iio_set_channel_offset(struct sol_iio_channel *channel, int offset)
{
    char path[PATH_MAX + 1], *pure_name = NULL;
    bool result;

    if (craft_filename_path(path, sizeof(path), CHANNEL_OFFSET_PATH,
        channel->device->device_id, channel->name)) {

        if (write_file(path, "%d", offset)) {
            channel->offset = offset;
            return true;
        }
    }

    /* If failed, try channel pure name */
    pure_name = channel_get_pure_name(channel);
    if (pure_name && craft_filename_path(path, sizeof(path), CHANNEL_OFFSET_PATH,
        channel->device->device_id, channel->name)) {

        result = write_file(path, "%d", offset);
        if (result)
            channel->offset = offset;
    }

    if (!result)
        SOL_WRN("Could not set offset to %d on channel [%s] of device%d",
            offset, channel->name, channel->device->device_id);

    free(pure_name);

    return result;
}

bool
sol_iio_device_start_buffer(struct sol_iio_device *device)
{
    /* Enable device after added all channels */
    if (device->buffer_enabled && !set_buffer_enabled(device, true)) {
        SOL_WRN("Could not enable buffer for device. No readings will be performed");
        return false;
    }

    return true;
}
