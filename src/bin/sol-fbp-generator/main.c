...
add_metatype_file_reader(const char *filename, struct sol_file_reader *reader)
{
    ...
    return 0;
err_exit:
    (void)sol_vector_del_element(&metatype_file_readers, rdata);
    return -ENOMEM;
}
...
print_usage(const char *program)
{
    fprintf(stderr, "usage: %s [-c CONF] [-j DESC -j DESC...] [-s SYMBOL] INPUT OUTPUT\n"
        ...
        "\n"
        "Note:\n"
        "    Soletta install directory will also be looked for node type description\n"
        "    files in addition to any `-j` argument, unless environmment variable\n"
        "    SOL_FLOW_DESCRIPTIONS_PATH is set. In this case, paths on this variable\n"
        "    will be looked in addition to any `-j` argument.\n"
        "    SOL_FLOW_DESCRIPTIONS_PATH may contain multiple paths separated by ':'.\n"
        "\n",
        program);
}
...
static void
handle_descriptions_paths(const char *descriptions_paths)
{
    struct sol_str_slice paths = sol_str_slice_from_str(descriptions_paths);
    struct sol_str_slice token;
    ...
    while (sol_str_slice_split_iterate(paths, &token, &itr, delim)) {
        ...
        if (!path) {
            SOL_WRN("Could not parse paths from SOL_FLOW_DESCRIPTIONS_PATH environment variable");
            return;
        }
...
main(int argc, char *argv[])
{
    ...
    /* Let's look for node descriptions environment variable SOL_FLOW_DESCRIPTIONS_PATH */
    descriptions_paths = getenv("SOL_FLOW_DESCRIPTIONS_PATH");
    if (descriptions_paths && descriptions_paths[0] != '\0') {
        handle_descriptions_paths(descriptions_paths);
    } else {
